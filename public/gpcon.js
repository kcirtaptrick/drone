...
(function () {
	 ...
	 */
	 function updateUI() {
		...
		let i = currentVisibleController;
		let gp = gamepads[i];
		let j;
		...
		for (j = 0; j < gp.buttons.length; j++) {
			...
			// Add button value to gpState
			gpState.buttons.push(button.value);
		}
		...
		for (j = 0; j < axesBoxCount; j++) {
			...
			if (!(j == axesBoxCount - 1 && gp.axes.length % 2 == 1)) {
				...
				if (mode == 'clamp') {
					...
					gpState.axis.push([clampX, clampY]);
					...
					gpState.axis.push([valueX, valueY]);
				}
			}

			axisPairValue.innerHTML = valueStr;
		}
		socket.emit('gamepadState', gpState);
	}
